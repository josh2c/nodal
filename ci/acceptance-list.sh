...
#   5. The verdict, which is what the list answers with in a checkout Nodal holds no row
#      for. A fixture of six worktrees checks every column against a worktree built to
#      have that one fact, and the cost is measured at twelve worktrees and at fifty. The
#      numbers are printed and nothing is gated on them, for the reason given above.
#
# The proof that the verdict writes nothing is in the safety suite, not here.
#
...
cargo test --locked -p nodal-cli --test verdict -- --nocapture
...
TMPDIR="$work/by-another-name" cargo test --locked -p nodal-cli --test verdict
TMPDIR="$work/by-another-name" cargo test --locked -p nodal-core --test list

echo "acceptance (list): every shape gets its verdict, the verdict table prints in a checkout nodal has never seen, both run under a linked path, and each is timed"
