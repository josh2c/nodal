...
//!
//! EV-2 and EV-3, met by the comparison harness in ordinary use. A project whose recipe
//! declares a `post_new` hook refused every `nodal new` until the hook was approved, and
//! the only command that wrote an approval was `nodal init`. `nodal init` refuses when
//! `nodal.toml` is there, so the one route through was `nodal init --force`, which
//! rewrites the recipe and replaces every comment in it with the template's own. A
//! person answering "yes, run this line" lost the notes they had written.
//!
...
/// The whole of EV-2 and EV-3, in one test: the refusal, the verb it names, and the
/// recipe that is untouched afterwards.
#[test]
...
fn a_command_that_changed_is_refused_until_it_is_approved_again() {
    ...
    let machine = Machine::new();
    write_project(root);
    assert!(nodal(&machine, root, &["approve"]).status.success());
