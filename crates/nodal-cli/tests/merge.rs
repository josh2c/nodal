...
/// Insist that the merge succeeded and removed the home itself.
fn assert_removed(merged: &std::process::Output) {
...
fn the_commits_a_squash_folded_stay_on_the_premerge_ref() {
    ...
    drop(workspace.nodal(&["merge", "worker-import", "--yes", "-m", "one squashed commit"]));
...
fn no_squash_puts_every_commit_of_the_branch_on_the_target() {
    ...
    drop(workspace.nodal(&["merge", "worker-import", "--yes", "--no-squash"]));
...
fn no_rebase_refuses_a_target_that_has_moved_and_a_rebase_takes_it() {
    ...
    drop(workspace.nodal(&["merge", "worker-import", "--yes"]));
    let log = workspace.main_log();
