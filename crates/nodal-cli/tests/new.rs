...
/// The project this suite makes units in, and the ways it needs one.
trait Creating {
    /// A project with `BULK_FILES` committed files, so that cloning it is long enough
    /// to be interrupted.
    fn with_bulk() -> Workspace;

    /// A two-package project whose base has been built and then left holding the caches
    ...
    fn with_a_base_that_holds_caches() -> Workspace;
...
impl Creating for Workspace {
    fn with_bulk() -> Self {
        Self::laid_out(state::BINARY, &Layout { bulk: BULK_FILES, ..Layout::default() })
    }

    fn with_a_base_that_holds_caches() -> Self {
        let workspace = Self::new(state::BINARY);
        workspace.commit_source();
        drop(stdout(&workspace.nodal(&["base", "build"])));
    ...
    fn commit_source(&self) {
        ...
        .unwrap();
        git_ok(&self.source, &["add", "-A"]);
        git_ok(&self.source, &["commit", "-qm", "the web package"]);
    }
}
...
fn a_new_unit_has_a_clean_status_and_a_checkout_of_its_own() {
    ...
    let home = workspace.homes().pop().unwrap();
    assert_eq!(
...
fn a_second_unit_on_one_branch_is_refused_and_the_holder_is_named() {
    ...
    assert_eq!(workspace.homes().len(), 1, "the refusal left no home behind");
}
...
fn two_units_are_two_working_copies_that_cannot_see_each_other() {
    ...
    drop(stdout(&workspace.nodal(&["new", "payroll export"])));

    let homes = workspace.homes();
...
fn the_second_unit_of_a_workspace_reuses_the_base_the_first_one_paid_for() {
    ...
    assert_eq!(workspace.homes().len(), 2, "and two units on it");
...
fn a_unit_carries_the_caches_that_move_and_none_of_the_caches_that_do_not() {
    ...
    drop(stdout(&workspace.nodal(&["new", "--name", "cache-check"])));
    let home = workspace.homes().pop().unwrap();
...
fn a_unit_made_from_a_base_with_no_stale_cache_has_nothing_to_report() {
    let workspace = Workspace::new(state::BINARY);
    drop(stdout(&workspace.nodal(&["new", "--name", "nothing-to-say"])));
