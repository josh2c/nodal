//! Which remotes a repository names, and which repository a checkout is a clone of.
//!
//! It used to answer a third question — how much of a revision the remotes already have,
//! from `rev-list <rev> --not --remotes`. That reads the repository's own record of its own
//! pushes, which nothing corrects when a branch is deleted on the remote, so it called work
//! pushed that no remote held. The question is answered by the witness doctrine now
//! ([`crate::doctor::unique::believed`]) and this module does not offer a second answer to
//! it.
//!
