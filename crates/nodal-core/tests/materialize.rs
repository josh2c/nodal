...
/// Clone `source` with every backend in turn, and check what each one made.
///
/// Every test here is one claim about a clone, made twice: once on the backend this
/// machine selects and once on the fallback. The two runs and the destination each one
/// writes to were spelled out in every test, which is nine copies of a loop and one
/// place for a run to be left out of by accident. The claim is the closure; the runs
/// are here.
///
/// The closure is given the label of the run it is in, so every message says which
/// backend the claim failed on.
fn each_backend(
    case: &Case,
    source: &Path,
    exclude: &Excludes,
    mut check: impl FnMut(&str, &Path, &Report),
) {
    for (label, backend) in backends() {
        let destination = case.destination(label);
        let report = backend.clone_tree(source, &destination, exclude).unwrap();
        check(label, &destination, &report);
    }
}
...
fn the_exclusion_list_leaves_out_what_it_names() {
    ...
    each_backend(&case, &source, &Excludes::default_list(), |label, clone, report| {
...
fn a_recipe_adds_to_what_a_clone_leaves_out() {
    ...
    each_backend(&case, &source, &exclude, |label, clone, _report| {
...
fn modes_and_modification_times_are_the_ones_the_source_had() {
    ...
    each_backend(&case, &source, &Excludes::default(), |label, clone, _report| {
...
fn both_backends_produce_the_same_tree() {
    ...
    let mut shapes: Vec<(String, BTreeMap<PathBuf, String>)> = Vec::new();
    each_backend(&case, &source, &Excludes::default_list(), |label, clone, _report| {
        shapes.push((label.to_owned(), shape(clone)));
    });
