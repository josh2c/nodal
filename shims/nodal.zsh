...
# the PATH answers. `whence -p` reads the PATH and never this function, which
...
__nodal_program() {
  ...
  whence -p nodal 2> /dev/null && return 0
...
__nodal_enter() {
  ...
  exports="$("$program" env --export --shell zsh "$1")" || return 1
  eval "$exports"
  __nodal_entered="$1"
}
