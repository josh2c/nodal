...
//! worried. It reports nested worktrees, stale caches, orphan databases and units, each
//! with a size, and it removes nothing (`decisions/DL-015`). A read command that wrote
//! would be worse than one that refused: the person ran it to find out what is there,
//! not to change it.
//!
//! "Nothing changed" is asserted byte for byte, on both trees doctor reads: the person's
//! checkout, and Nodal's whole state directory with every home, base and trashed home in
//! it. The registry file is the one thing left out, and it is left out for a stated
...
/// The nested worktree planted in the checkout, so doctor has something to report.
...
fn doctor_leaves_the_checkout_and_the_state_directory_byte_for_byte_as_it_found_them() {
    ...
    // A leftover for the report to be about, so that the command is not reporting on an
    ...
    source.assert_unchanged(&Snapshot::of(&machine.source), "doctor wrote in the checkout");
...
fn the_json_rendering_leaves_the_machine_alone_as_well() {
    ...
    source.assert_unchanged(&Snapshot::of(&machine.source), "doctor --json wrote in the checkout");
    state.assert_unchanged(
        &Snapshot::of_except(&machine.state, is_registry),
